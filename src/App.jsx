...
function RankBadge({ sales = 0, size = "sm" }) {
  ...
  return (
    <span style={{ display:"inline-flex",alignItems:"center",gap:lg?5:4,border:`1px solid ${r.color}55`,borderRadius:3,padding:lg?"4px 10px":"2px 7px",fontFamily:"'Manrope',sans-serif",fontSize:lg?10:9,color:r.color,fontWeight:700,letterSpacing:1.5,textTransform:"uppercase",whiteSpace:"nowrap",background:r.color+"08" }}>
...
function Ava({ u, size=38 }) {
  ...
  return (
    <div style={{ position:"relative",flexShrink:0 }}>
      <div style={{ width:size,height:size,borderRadius:"50%",background:`linear-gradient(135deg,${B.warm},${B.border})`,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Playfair Display',serif",fontWeight:700,fontSize:size*.34,color:T.muted,border:`1.5px solid ${rank.color}40` }}>{initials}</div>
      {size>=36&&<span style={{ position:"absolute",bottom:-2,right:-2,fontSize:size*0.28,lineHeight:1,background:B.canvas,borderRadius:"50%",padding:"1px",color:rank.color }}>{rank.icon}</span>}
    </div>
  );
}
...
function BottleSlider({ value=90, onChange }) {
  ...
  const liq=bottleLiq(pct);const liqH=(pct/100)*108,liqY=40+(108-liqH);
  return (
        ...
        <svg width="80" height="160" viewBox="0 0 80 160" fill="none">
          <defs>
            <linearGradient id="sl-l" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor={liq.top} stopOpacity=".9"/><stop offset="60%" stopColor={liq.mid} stopOpacity=".85"/><stop offset="100%" stopColor={liq.bot} stopOpacity=".9"/>
            </linearGradient>
            <linearGradient id="sl-g" x1="0" y1="0" x2="1" y2="0">
              <stop offset="0%" stopColor="#fff" stopOpacity=".12"/><stop offset="30%" stopColor="#fff" stopOpacity=".22"/><stop offset="100%" stopColor="#fff" stopOpacity=".04"/>
            </linearGradient>
      ...
      <div style={{ flex:1 }}>
        ...
        <div style={{ display:"flex",gap:6,marginTop:16,flexWrap:"wrap" }}>
          {[{label:"Bontatlan",v:100},{label:"~¾",v:75},{label:"~½",v:50},{label:"~¼",v:25}].map(({label,v})=>(
            <button key={v} onClick={()=>onChange(v)} style={{ background:pct===v?ACC.goldPale:"transparent",border:`1px solid ${pct===v?ACC.gold:B.border}`,color:pct===v?ACC.gold:T.muted,padding:"5px 12px",borderRadius:4,cursor:"pointer",fontFamily:"'Manrope',sans-serif",fontSize:10,letterSpacing:.5,transition:"all .15s" }}>{label}</button>
          ))}
        </div>
    ...
    <nav style={{ position:"fixed",top:0,left:0,right:0,zIndex:100,height:62,background:scrolled?"rgba(250,248,244,.96)":B.canvas,backdropFilter:"blur(16px)",borderBottom:`1px solid ${scrolled?B.border:"transparent"}`,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 40px",transition:"border-color .3s,background .3s" }}>
      ...
      <div style={{ display:"flex",gap:2,alignItems:"center" }}>
        {[["home","Főoldal"],["market","Piac"]].map(([p,l])=>(
          <button key={p} onClick={()=>go(p)} style={{ background:"transparent",border:"none",color:page===p?T.heading:T.muted,padding:"8px 14px",borderRadius:6,cursor:"pointer",fontFamily:"'Manrope',sans-serif",fontSize:12,fontWeight:page===p?700:500,letterSpacing:.3,borderBottom:page===p?`2px solid ${ACC.gold}`:"2px solid transparent",transition:"all .15s" }}>{l}</button>
        ))}
        <button onClick={()=>go("sell")} style={{ background:ACC.ink,border:"none",color:B.canvas,padding:"8px 18px",borderRadius:6,cursor:"pointer",fontFamily:"'Manrope',sans-serif",fontSize:12,fontWeight:700,letterSpacing:.3,marginLeft:8 }}>+ Hirdetés</button>
        {profile ? (
          <>
...
function Card({ l, u, onClick }) {
  ...
  return (
    <div onClick={onClick} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      ...
      {(isSold||isPend)&&(
        <div style={{ position:"absolute",top:0,left:0,right:0,padding:"4px 0",textAlign:"center",background:isSold?"#f2faf6":"#fefaf0",borderBottom:`1px solid ${isSold?ACC.green:ACC.gold}28`,fontFamily:"'Manrope',sans-serif",fontSize:9,letterSpacing:1.5,color:isSold?ACC.green:ACC.gold,fontWeight:700 }}>
          {isSold?"✓ ELADVA":"⏳ FÜGGŐBEN"}
        </div>
      )}
...
function Detail({ l, u, curProfile, go, setProfileId, setActiveChatWith, onStatusChange }) {
  ...
  async function confirmSold(){
    ...
    try{
      const{data:bp}=await supabase.from("profiles").select("email,name").eq("id",selectedBuyer.id).single();
      if(bp?.email)await supabase.functions.invoke("clever-worker",{body:{to_email:bp.email,from_name:curProfile.name,message_text:`Sikeresen megkaptad: ${l.brand} ${l.name}\n\nKérlek értékeld az eladót!`,cta_url:"https://scentrade.vercel.app",cta_text:"Értékelés írása →"}});
      ...
      <div style={{ display:"grid",gridTemplateColumns:"1fr 300px",gap:52 }}>
        ...
        <div style={{ display:"flex",flexDirection:"column",gap:12 }}>
          ...
          {!isOwn&&status==="sold"&&l.buyer_id===curProfile?.id&&(
            <button onClick={()=>{setReviewTarget(u);setShowReviewModal(true);}} style={{ background:ACC.ink,border:"none",color:B.canvas,padding:"14px",borderRadius:7,cursor:"pointer",fontFamily:"'Manrope',sans-serif",fontSize:14,fontWeight:700 }}>⭐ Értékelem az eladót</button>
          )}
          ...
          <div style={{ border:`1px solid ${B.border}`,borderRadius:7,padding:"16px 18px",background:B.paper }}>
            ...
            {["Valódi értékelések","PM alapú egyeztetés","Hitelesített jelvény","Átverés bejelentés"].map(txt=>(
              <div key={txt} style={{ display:"flex",gap:10,fontSize:12,color:T.muted,marginBottom:7,fontFamily:"'Manrope',sans-serif" }}><span style={{ color:ACC.gold,flexShrink:0 }}>✓</span>{txt}</div>
            ))}
          </div>
        </div>
      </div>
        ...
        <Modal onClose={()=>setShowSoldModal(false)}>
          ...
          <div style={{ marginBottom:20 }}>
            ...
            <div style={{ position:"relative" }}>
              <input value={buyerSearch} onChange={e=>searchBuyer(e.target.value)} placeholder="pl. kovacs_balazs" autoComplete="off"
              ...
              <div style={{ marginTop:6,display:"flex",flexDirection:"column",gap:5 }}>
                {searchResults.map(p=>(
                  <div key={p.id} onClick={()=>{setSelectedBuyer(p);setBuyerSearch(p.name);setSearchResults([]);}}
                    style={{ display:"flex",alignItems:"center",gap:12,padding:"10px 13px",borderRadius:7,cursor:"pointer",background:selectedBuyer?.id===p.id?ACC.goldPale:B.paper,border:`1px solid ${selectedBuyer?.id===p.id?ACC.gold:B.border}` }}>
                    <Ava u={p} size={32}/><div><div style={{ fontFamily:"'Playfair Display',serif",fontSize:15,color:T.heading }}>{p.name}</div><RankBadge sales={p.sales||0}/></div>
                    {selectedBuyer?.id===p.id&&<span style={{ color:ACC.gold,fontSize:16,marginLeft:"auto" }}>✓</span>}
                  </div>
                ))}
              </div>
            )}
          ...
          <div style={{ display:"flex",alignItems:"center",gap:12,marginBottom:20 }}>
            <div style={{ flex:1,height:1,background:B.border }}/><span style={{ fontFamily:"'Manrope',sans-serif",fontSize:10,color:T.faint,letterSpacing:2,fontWeight:600 }}>VAGY</span><div style={{ flex:1,height:1,background:B.border }}/>
          </div>
            ...
            <div style={{ display:"flex",flexDirection:"column",gap:6,marginBottom:22,maxHeight:220,overflowY:"auto" }}>
              ...
              {msgPartners.map(p=>(
                <div key={p.id} onClick={()=>{setSelectedBuyer(p);setBuyerSearch("");setSearchResults([]);}}
                  style={{ display:"flex",alignItems:"center",gap:12,padding:"11px 13px",borderRadius:7,cursor:"pointer",background:selectedBuyer?.id===p.id?ACC.goldPale:B.paper,border:`1px solid ${selectedBuyer?.id===p.id?ACC.gold:B.border}`,transition:"all .15s" }}>
                  <Ava u={p} size={34}/><div style={{ flex:1 }}><div style={{ fontFamily:"'Playfair Display',serif",fontSize:15,color:T.heading }}>{p.name}</div><RankBadge sales={p.sales||0}/></div>
          ...
          {selectedBuyer&&(
            <div style={{ background:ACC.greenPale,border:`1px solid ${ACC.green}30`,borderRadius:7,padding:"11px 14px",marginBottom:14,display:"flex",alignItems:"center",gap:12 }}>
              <Ava u={selectedBuyer} size={30}/>
              <div><div style={{ fontFamily:"'Manrope',sans-serif",fontSize:9,color:ACC.green,letterSpacing:1.5,marginBottom:2,fontWeight:700 }}>KIVÁLASZTOTT VEVŐ</div><div style={{ fontFamily:"'Playfair Display',serif",fontSize:15,color:T.heading }}>{selectedBuyer.name}</div></div>
          ...
          <button onClick={confirmSold} disabled={!selectedBuyer}
            style={{ background:selectedBuyer?ACC.green:B.warm,border:"none",color:selectedBuyer?"#fff":T.faint,padding:"14px",width:"100%",borderRadius:7,cursor:selectedBuyer?"pointer":"not-allowed",fontFamily:"'Manrope',sans-serif",fontSize:14,fontWeight:700 }}>
      ...
      {showReviewModal&&reviewTarget&&(
        <Modal onClose={()=>setShowReviewModal(false)}>
          ...
          <textarea ref={reviewRef} defaultValue="" rows={4} placeholder="Hogyan ment az üzlet? Csomagolás, gyorsaság, kommunikáció..."
            style={{ background:B.paper,border:`1px solid ${B.border}`,color:T.body,padding:"13px 15px",borderRadius:7,width:"100%",fontFamily:"'Manrope',sans-serif",fontSize:14,resize:"vertical",outline:"none",boxSizing:"border-box",marginBottom:16,lineHeight:1.7 }}/>
...
function Profile({ pu, curProfile, go, listings, setActiveChatWith, onSignOut }) {
  ...
  async function addWish(){
    const val=wishRef.current?.value?.trim();if(!val||!curProfile)return;
    const[brand,...rest]=val.split(" ");
    const{data}=await supabase.from("wishlists").insert({user_id:curProfile.id,brand,name:rest.join(" ")||brand,raw:val}).select().single();
    ...
    <div style={{ paddingTop:62,maxWidth:980,margin:"0 auto",padding:"80px 40px",background:B.canvas }}>
      ...
      <div style={{ display:"flex",borderBottom:`1px solid ${B.border}`,marginBottom:36 }}>
        {[["listings",`Hirdetések (${uls.length})`],["reviews",`Értékelések (${reviews.length})`],["wishlist",`Kívánlista (${wishlist.length})`]].map(([k,l])=>(
          <button key={k} onClick={()=>setTab(k)} style={{ background:"transparent",border:"none",borderBottom:tab===k?`2px solid ${ACC.gold}`:"2px solid transparent",color:tab===k?T.heading:T.faint,padding:"12px 20px",cursor:"pointer",fontFamily:"'Manrope',sans-serif",fontSize:12,fontWeight:tab===k?700:500,letterSpacing:.3,marginBottom:-1 }}>{l}</button>
        ))}
      </div>
      ...
      {tab==="wishlist"&&(
        <div>
          <div style={{ border:`1px solid ${B.border}`,borderRadius:8,padding:"22px 26px",marginBottom:24,background:B.paper }}>
            ...
            <p style={{ fontFamily:"'Manrope',sans-serif",fontSize:12,color:T.muted,lineHeight:1.8 }}>Ha valaki feltölti valamelyik keresett parfümodet, automatikus értesítést kapsz e-mailben.</p>
          </div>
          {isOwn&&(
            <div style={{ display:"flex",gap:10,marginBottom:22 }}>
              <input ref={wishRef} placeholder="pl. Creed Aventus  vagy  Dior Sauvage" defaultValue="" autoComplete="off" autoCorrect="off" spellCheck="false"
...
function Messages({ curProfile, activeChatWith, setActiveChatWith }) {
    ...
    const sub=supabase.channel("msg-rt-"+curProfile.id)
      .on("postgres_changes",{event:"INSERT",schema:"public",table:"messages",filter:`to_user=eq.${curProfile.id}`},payload=>{
        if(activeChatWith===payload.new.from_user){
          supabase.from("messages").update({read:true,read_at:new Date().toISOString()}).eq("id",payload.new.id).then(()=>loadChat(activeChatWith));
        }else loadConvs();
      })
  ...
  async function send(){
    ...
    try{
      ...
      if(partner?.email)await supabase.functions.invoke("clever-worker",{body:{to_email:partner.email,to_name:partner.name,from_name:curProfile.name,message_text:txt}});
    }catch(e){console.warn(e);}
        ...
        {convs.map(({partnerId,lastMsg})=>{
          ...
          return(
            <div key={partnerId} onClick={()=>{setActiveChatWith(partnerId);setMobileView("chat");}}
              style={{ padding:"14px 18px",cursor:"pointer",display:"flex",gap:12,alignItems:"center",background:active?B.warm:isUnread?B.paper:"transparent",borderLeft:active?`3px solid ${ACC.gold}`:isUnread?`3px solid ${ACC.gold}55`:"3px solid transparent",borderBottom:`1px solid ${B.border}` }}>
      ...
      {activeChatWith?(
        <>
          <div style={{ padding:"14px 20px",borderBottom:`1px solid ${B.border}`,display:"flex",alignItems:"center",gap:12,background:B.paper,flexShrink:0 }}>
            ...
            <div style={{ fontFamily:"'Playfair Display',serif",fontSize:18,color:T.heading,flex:1 }}>{activePartner?.name||"…"}</div>
  ...
  return(
    <div style={{ paddingTop:62,height:"100dvh",display:"flex",overflow:"hidden" }}>
      {listPane}{chatPane}
    </div>
  );
}
...
function Sell({ curProfile, go, setListings, showToast }) {
  ...
  const refBrand=useRef(null);const refName=useRef(null);const refPrice=useRef(null);
  const refDesc=useRef(null);const refTags=useRef(null);

  async function uploadImages(lid){
    const urls=[];
    for(const img of images){
      const ext=img.name.split(".").pop();const path=`${curProfile.id}/${lid}/${Date.now()}.${ext}`;
  ...
  async function submit(){
    ...
    const{data,error}=await supabase.from("listings").insert({
      ...
      condition:(!isDec&&sType==="sell")?sCondition:null,price:Number(price),
      decant_ml:isDec?Number(sDecantMl):null,description:desc,category:sCategory,
      tags:tags.split(",").map(t=>t.trim()).filter(Boolean),icon:sIcon,views:0,favorites:0,status:"active",swap_ok:sSwap,
    }).select().single();
    ...
    try{
      const{data:matches}=await supabase.from("wishlists").select("*, profiles(email, name)").neq("user_id",curProfile.id).or(`raw.ilike.%${brand}%,raw.ilike.%${name}%`);
      if(matches?.length>0){
        for(const match of matches){
          if(!match.profiles?.email)continue;
          await supabase.functions.invoke("clever-worker",{body:{to_email:match.profiles.email,from_name:"Scentrade",message_text:`🌟 Megjelent az álomillatod!\n\n${curProfile.name} épp most hirdette meg: ${brand} ${name}`,subject_override:`🌟 Megjelent: ${brand} ${name}`,cta_url:"https://scentrade.vercel.app",cta_text:"Megnézem most →"}});
        }
      }
    }catch(e){console.warn(e);}
  ...
  return(
    <div style={{ paddingTop:62,maxWidth:700,margin:"0 auto",padding:"80px 24px 100px",background:B.canvas }}>
      ...
      <SellField label="Tagek (vesszővel)">
        <input ref={refTags} defaultValue="" placeholder="creed, niche, woody" autoComplete="off" spellCheck="false" style={inp}/>
      </SellField>
          ...
          <div style={{ display:"flex",gap:8,marginTop:10,flexWrap:"wrap" }}>
            {images.map((img,i)=>(
              <div key={i} style={{ position:"relative" }}>
                <img src={URL.createObjectURL(img)} style={{ width:80,height:80,objectFit:"cover",borderRadius:7,border:`1px solid ${B.border}` }} alt=""/>
                <button onClick={()=>setImages(p=>p.filter((_,j)=>j!==i))} style={{ position:"absolute",top:-6,right:-6,background:ACC.red,border:"none",borderRadius:"50%",width:20,height:20,cursor:"pointer",color:"white",fontSize:12,display:"flex",alignItems:"center",justifyContent:"center" }}>×</button>
              </div>
            ))}
          </div>
        )}
...
function Login({ go, showToast }) {
  ...
  async function doRegister(){
    ...
    setLoading(false);
    if(sd.session){showToast("Sikeres regisztráció! Üdv a Scentrade-n!","success");go("home");}
    else setRegState("confirm");
  }
  ...
  return(
    <div style={{ paddingTop:62,minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",padding:"80px 24px",background:B.canvas }}>
      <div style={{ border:`1px solid ${B.border}`,borderRadius:12,padding:"56px 48px",width:"100%",maxWidth:440,boxShadow:"0 8px 40px rgba(0,0,0,.06)",background:B.paper }}>
        ...
        <p style={{ color:T.faint,fontSize:11,fontFamily:"'Manrope',sans-serif",textAlign:"center",cursor:"pointer",letterSpacing:.5,fontWeight:600 }} onClick={()=>sw(mode==="login"?"register":"login")}>
...
export default function App() {
  ...
  async function signOut(){
    ...
    setPage("home");setSelId(null);setProfileId(null);setACW(null);setUnread(0);
    window.scrollTo(0,0);
  }
  ...
  if(loading)return(
    <div style={{ background:B.canvas,minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center" }}>
      <span style={{ fontFamily:"'Playfair Display',serif",fontSize:28,color:ACC.gold,letterSpacing:4,animation:"pulse 1.2s ease infinite" }}>SCENTRADE</span>
    </div>
  );

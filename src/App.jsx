        ...
        {profile ? (
          <>
            <button onClick={() => go("messages")} style={{ background:"transparent", border:"none", cursor:"pointer", color:"#6a5a40", fontSize:16, position:"relative", padding:"4px 9px" }}>
              ✉
              {unreadCount > 0 && <span style={{ position:"absolute", top:2, right:5, background:"#c9952a", borderRadius:"50%", width:7, height:7 }} />}
            </button>
    ...
    <div style={{ paddingTop:58, maxWidth:980, margin:"0 auto", padding:"80px 36px 80px" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 310px", gap:40 }}>
        <div>
          ...
          <p style={{ color:"#4a3820", marginBottom:24 }}>{isDecant ? `${l.decant_ml}ml spray dekant` : `${l.size||""}${l.fill ? ` · ${l.fill}% tele` : ""}`}</p>
...
function Messages({ curProfile, profiles, activeChatWith, setActiveChatWith }) {
  ...
  useEffect(() => {
    ...
    const sub = supabase.channel("messages")
      .on("postgres_changes", { event:"INSERT", schema:"public", table:"messages" }, () => { loadConversations(); if (activeChatWith) loadChat(activeChatWith); }).subscribe();
  ...
  async function loadConversations() {
    ...
    data.forEach(m => { const partner = m.from_user===curProfile.id ? m.to_user : m.from_user; if (!seen.has(partner)) { seen.add(partner); convs.push({ partnerId:partner, lastMsg:m }); } });
  ...
  async function send() {
    ...
    await supabase.from("messages").insert({ from_user:curProfile.id, to_user:activeChatWith, text:newMsg.trim() });
    const toUser = profiles[activeChatWith];
    if (toUser?.email) await supabase.functions.invoke("send-message-email", { body:{ to_email:toUser.email, to_name:toUser.name, from_name:curProfile.name, message_text:newMsg.trim() } });
  ...
  return (
    <div style={{ paddingTop:58, height:"100vh", display:"flex" }}>
      <div style={{ width:280, background:"#0c0a07", borderRight:"1px solid #1a1610", overflowY:"auto", flexShrink:0 }}>
        ...
        {conversations.length===0 && <p style={{ padding:"22px 18px", color:"#2e2418", fontFamily:"'DM Mono',monospace", fontSize:10 }}>Még nincs üzeneted.</p>}
        {conversations.map(({ partnerId, lastMsg }) => {
          const u = profiles[partnerId]; if (!u) return null;
          const active = activeChatWith===partnerId;
          return (
            <div key={partnerId} onClick={() => setActiveChatWith(partnerId)} style={{ padding:"14px 18px", cursor:"pointer", display:"flex", gap:11, alignItems:"center", background:active?"#131008":"transparent", borderLeft:active?"2px solid #c9952a":"2px solid transparent", borderBottom:"1px solid #131008" }}>
              <Ava u={u} size={36} />
        ...
        {activeChatWith && profiles[activeChatWith] ? (
          <>
              ...
              {chat.map((m,i) => {
                ...
                return (
                  <div key={i} style={{ display:"flex", justifyContent:me?"flex-end":"flex-start" }}>
                    <div style={{ maxWidth:"68%", padding:"10px 14px", background:me?"#1e1508":"#141009", border:`1px solid ${me?"#c9952a28":"#1e1a13"}`, borderRadius:me?"14px 14px 3px 14px":"14px 14px 14px 3px" }}>
                      <p style={{ color:"#f0e4cc", fontSize:14, lineHeight:1.6, margin:0 }}>{m.text}</p>
                      <p style={{ color:"#2e2418", fontSize:10, fontFamily:"'DM Mono',monospace", margin:"5px 0 0", textAlign:"right" }}>{new Date(m.created_at).toLocaleTimeString("hu-HU",{hour:"2-digit",minute:"2-digit"})}</p>
                    </div>
                  </div>
                );
              })}
            ...
            <div style={{ padding:"14px 22px", borderTop:"1px solid #1a1610", display:"flex", gap:10 }}>
              <input value={newMsg} onChange={e=>setNewMsg(e.target.value)} onKeyDown={e=>e.key==="Enter"&&send()} placeholder="Írj üzenetet... (Enter = küldés)"
...
function Sell({ curProfile, go, setListings, showToast }) {
  const [form, setForm] = useState({ type:"sell", listing_type:"full", brand:"", name:"", size:"100ml", fill:90, condition:"excellent", price:"", decant_ml:5, category:"woody", description:"", tags:"", icon:"✨" });
  ...
  async function submit() {
    ...
    const { data, error } = await supabase.from("listings").insert({
      ...
      size:isDecant?null:form.size, fill:(!isDecant&&form.type==="sell")?Number(form.fill):null,
      condition:(!isDecant&&form.type==="sell")?form.condition:null, price:Number(form.price),
      decant_ml:isDecant?Number(form.decant_ml):null, description:form.description.trim(), category:form.category,
      tags:form.tags.split(",").map(t=>t.trim()).filter(Boolean), icon:form.icon, views:0, favorites:0
    }).select().single();
  ...
  return (
    <div style={{ paddingTop:58, maxWidth:720, margin:"0 auto", padding:"80px 36px 90px" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
        <F label="Márka *" errKey="brand"><input value={form.brand} onChange={e=>setForm({...form,brand:e.target.value})} placeholder="pl. Creed" style={{...inp,...(errors.brand?errStyle:{})}} /></F>
        <F label="Név *" errKey="name"><input value={form.name} onChange={e=>setForm({...form,name:e.target.value})} placeholder="pl. Aventus" style={{...inp,...(errors.name?errStyle:{})}} /></F>
        {form.listing_type==="full" && <F label="Méret"><select value={form.size} onChange={e=>setForm({...form,size:e.target.value})} style={{...inp,cursor:"pointer"}}>{["30ml","50ml","75ml","100ml","125ml","150ml","200ml","Egyéb"].map(s=><option key={s}>{s}</option>)}</select></F>}
        {form.listing_type==="decant" && <F label="Dekant méret (ml)"><select value={form.decant_ml} onChange={e=>setForm({...form,decant_ml:e.target.value})} style={{...inp,cursor:"pointer"}}>{DECANT_SIZES.map(s=><option key={s} value={s}>{s}ml</option>)}</select></F>}
        <F label="Kategória"><select value={form.category} onChange={e=>setForm({...form,category:e.target.value})} style={{...inp,cursor:"pointer"}}>{["woody","oriental","floral","fresh","aromatic"].map(c=><option key={c}>{c}</option>)}</select></F>
      </div>
      {form.listing_type==="full" && form.type==="sell" && (
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
          <F label={`Teli szint: ${form.fill}%`}><input type="range" min={5} max={100} value={form.fill} onChange={e=>setForm({...form,fill:e.target.value})} style={{ width:"100%", accentColor:"#c9952a", marginTop:4 }} /></F>
          <F label="Állapot"><select value={form.condition} onChange={e=>setForm({...form,condition:e.target.value})} style={{...inp,cursor:"pointer"}}>{Object.entries(COND).map(([k,v])=><option key={k} value={k}>{v}</option>)}</select></F>
        </div>
      )}
      <F label="Ár (Ft) *" errKey="price"><input value={form.price} onChange={e=>setForm({...form,price:e.target.value})} placeholder="pl. 35000" type="number" style={{...inp,...(errors.price?errStyle:{})}} /></F>
      <F label="Leírás *" errKey="description"><textarea value={form.description} onChange={e=>setForm({...form,description:e.target.value})} rows={4} placeholder="Batch, állapot részletei, csere lehetőség..." style={{...inp,resize:"vertical",...(errors.description?errStyle:{})}} /></F>
      <F label="Tagek (vesszővel elválasztva)"><input value={form.tags} onChange={e=>setForm({...form,tags:e.target.value})} placeholder="creed, niche, woody" style={inp} /></F>
      <F label="Fotók (max 5)">
          ...
          <div style={{ display:"flex", gap:8, marginTop:10, flexWrap:"wrap" }}>
            {images.map((img,i) => (
              <div key={i} style={{ position:"relative" }}>
                <img src={URL.createObjectURL(img)} style={{ width:80, height:80, objectFit:"cover", borderRadius:8, border:"1px solid #221e18" }} />
      ...
      <F label="Ikon">
        <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
          {ICONS.map(ic => <button key={ic} onClick={() => setForm({...form,icon:ic})} style={{ background:form.icon===ic?"#c9952a18":"#141009", border:form.icon===ic?"1px solid #c9952a40":"1px solid #221e18", borderRadius:8, padding:"8px 12px", cursor:"pointer", fontSize:20 }}>{ic}</button>)}
        </div>
      </F>
...
export default function App() {
  ...
  return (
    <div style={{ background:"#0a0806", minHeight:"100vh", color:"#f0e4cc" }}>
      ...
      {page==="detail" && selListing && <Detail l={selListing} u={allProfiles[selListing.user_id]} curProfile={profile} go={go} setProfileId={setProfileId} setActiveChatWith={setActiveChatWith} />}

...
const ACC = { gold:"#b8943f", goldPale:"#b8943f18", goldMid:"#b8943f35", goldWarm:"#d4a84b", ink:"#1a1714", red:"#c0453a", redPale:"#c0453a12", green:"#3a7a52", greenPale:"#3a7a5212" };
...
function RankBadge({sales=0,size="sm"}) {
  const r=getRank(sales);const lg=size==="lg";
  return(<span style={{display:"inline-flex",alignItems:"center",gap:lg?5:4,border:`1px solid ${r.color}55`,borderRadius:3,padding:lg?"4px 10px":"2px 7px",fontFamily:"'Manrope',sans-serif",fontSize:lg?10:9,color:r.color,fontWeight:700,letterSpacing:1.5,textTransform:"uppercase",whiteSpace:"nowrap",background:r.color+"08"}}><span style={{fontSize:lg?9:8}}>{r.icon}</span>{r.label}</span>);
}
function RankProgress({sales=0}) {
  const r=getRank(sales);let next=null,pct=100;
  ...
  return(
    <div style={{border:`1px solid ${B.border}`,borderRadius:8,padding:"20px 24px",background:B.paper}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
        <RankBadge sales={sales} size="lg"/><span style={{fontFamily:"'Manrope',sans-serif",fontSize:12,color:T.muted}}>{sales} eladás</span>
      </div>
      {next&&(<><div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}><span style={{fontFamily:"'Manrope',sans-serif",fontSize:10,color:T.faint,letterSpacing:1}}>KÖVETKEZŐ: {next.label.toUpperCase()}</span><span style={{fontFamily:"'Manrope',sans-serif",fontSize:10,color:ACC.gold}}>{next.at-sales} eladás hiányzik</span></div><div style={{height:2,background:B.border,borderRadius:2}}><div style={{height:"100%",width:`${pct}%`,borderRadius:2,background:`linear-gradient(90deg,${ACC.gold}60,${ACC.gold})`,transition:"width .6s ease"}}/></div></>)}
...
function useAuth() {
  ...
  useEffect(()=>{
    supabase.auth.getSession().then(({data:{session}})=>{setUser(session?.user??null);if(session?.user)fetchProfile(session.user.id);else setLoading(false);});
...
function usePresence(userId) {
  ...
  useEffect(()=>{
    ...
    ch.on("presence",{event:"sync"},()=>{
      const state=ch.presenceState();const map={};
      Object.entries(state).forEach(([uid,p])=>{if(p?.length>0)map[uid]={online:true};});
      setPresenceMap(map);
    });
    ch.on("presence",{event:"join"},({key})=>setPresenceMap(p=>({...p,[key]:{online:true}})));
    ch.on("presence",{event:"leave"},({key})=>setPresenceMap(p=>({...p,[key]:{online:false,last_seen:new Date().toISOString()}})));
    ch.subscribe(async(status)=>{if(status==="SUBSCRIBED")await ch.track({online:true,user_id:userId});});
    const onUnload=()=>{try{ch.track({online:false,user_id:userId});}catch(e){}};
    window.addEventListener("beforeunload",onUnload);
    return()=>{window.removeEventListener("beforeunload",onUnload);ch.untrack();supabase.removeChannel(ch);};
  },[userId]);
  return presenceMap;
}
...
function OnlineBadge({presenceMap,userId,showLabel=false}) {
  ...
  if(isOnline)return(<span style={{display:"inline-flex",alignItems:"center",gap:4}}><span style={{width:7,height:7,borderRadius:"50%",background:ACC.green,boxShadow:`0 0 6px ${ACC.green}88`,flexShrink:0,display:"inline-block"}}/>{showLabel&&<span style={{fontFamily:"'Manrope',sans-serif",fontSize:10,color:ACC.green,fontWeight:700,letterSpacing:.5}}>Online</span>}</span>);
  if(lastSeen)return(<span style={{display:"inline-flex",alignItems:"center",gap:4}}><span style={{width:7,height:7,borderRadius:"50%",background:B.borderDk,flexShrink:0,display:"inline-block"}}/>{showLabel&&<span style={{fontFamily:"'Manrope',sans-serif",fontSize:10,color:T.faint,letterSpacing:.5}}>utoljára: {relTime(lastSeen)}</span>}</span>);
  return null;
}
...
function Ava({u,size=38}) {
  ...
  return(<div style={{position:"relative",flexShrink:0}}><div style={{width:size,height:size,borderRadius:"50%",background:`linear-gradient(135deg,${B.warm},${B.border})`,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Playfair Display',serif",fontWeight:700,fontSize:size*.34,color:T.muted,border:`1.5px solid ${rank.color}40`}}>{initials}</div>{size>=36&&<span style={{position:"absolute",bottom:-2,right:-2,fontSize:size*0.28,lineHeight:1,background:B.canvas,borderRadius:"50%",padding:"1px",color:rank.color}}>{rank.icon}</span>}</div>);
}
...
function relTime(dateStr) {
  ...
  if(m<1)return"most";if(m<60)return`${m}p`;if(h<24)return`${h}ó`;if(day<7)return`${day}n`;
  return new Date(dateStr).toLocaleDateString("hu-HU",{month:"short",day:"numeric"});
}
function Toast({message,type="error",onClose}) {
  useEffect(()=>{const t=setTimeout(onClose,4500);return()=>clearTimeout(t);},[onClose]);
  const cfg={error:{bg:"#fff5f5",border:`${ACC.red}30`,text:ACC.red},success:{bg:"#f2faf6",border:`${ACC.green}30`,text:ACC.green},info:{bg:"#faf8f0",border:`${ACC.gold}30`,text:ACC.gold}};
  const c=cfg[type]||cfg.error;
  return(<div style={{background:c.bg,border:`1px solid ${c.border}`,borderRadius:8,padding:"14px 18px",maxWidth:360,boxShadow:"0 4px 24px rgba(0,0,0,.1)",display:"flex",alignItems:"flex-start",gap:10}}><span style={{color:c.text,fontSize:14,flexShrink:0}}>{type==="success"?"✓":type==="info"?"◆":"✕"}</span><p style={{color:c.text,fontFamily:"'Manrope',sans-serif",fontSize:13,lineHeight:1.6,flex:1}}>{message}</p><button onClick={onClose} style={{background:"none",border:"none",color:c.text,cursor:"pointer",fontSize:16,opacity:.4,padding:0}}>×</button></div>);
}
function useToast() {
  const [toasts,setToasts]=useState([]);
  const show=(message,type="error")=>{const id=Date.now();setToasts(p=>[...p,{id,message,type}]);};
  const remove=id=>setToasts(p=>p.filter(t=>t.id!==id));
  const ToastContainer=()=>(<div style={{position:"fixed",bottom:28,right:28,zIndex:9999,display:"flex",flexDirection:"column",gap:8}}>{toasts.map(t=><Toast key={t.id} message={t.message} type={t.type} onClose={()=>remove(t.id)}/>)}</div>);
  return{show,ToastContainer};
}

function bottleLiq(pct) {
  if(pct>=80)return{top:"#d4a84b",mid:"#b8943f",bot:"#8a6e28"};if(pct>=50)return{top:"#c9924a",mid:"#a87030",bot:"#7a4e18"};
  if(pct>=20)return{top:"#c0854a",mid:"#9a6228",bot:"#6e4010"};return{top:"#b07040",mid:"#8a5020",bot:"#5c300a"};
}
function fillLbl(pct){if(pct===100)return"Bontatlan";if(pct>=90)return"Szinte tele";if(pct>=75)return"Háromnegyedes";if(pct>=50)return"Feles";if(pct>=25)return"Negyed körüli";if(pct>=10)return"Kevés maradt";return"Majdnem üres";}

function BottleSlider({value=90,onChange}) {
  const pct=Math.min(100,Math.max(1,value));const liq=bottleLiq(pct);const liqH=(pct/100)*108,liqY=40+(108-liqH);
  return(
    <div style={{border:`1px solid ${B.border}`,borderRadius:10,padding:"24px 28px",display:"flex",gap:32,alignItems:"center",background:B.paper}}>
      <div style={{flexShrink:0,filter:"drop-shadow(0 8px 20px rgba(0,0,0,.08))"}}>
        <svg width="80" height="160" viewBox="0 0 80 160" fill="none">
          <defs>
            <linearGradient id="sl-l" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={liq.top} stopOpacity=".9"/><stop offset="60%" stopColor={liq.mid} stopOpacity=".85"/><stop offset="100%" stopColor={liq.bot} stopOpacity=".9"/></linearGradient>
            <linearGradient id="sl-g" x1="0" y1="0" x2="1" y2="0"><stop offset="0%" stopColor="#fff" stopOpacity=".12"/><stop offset="30%" stopColor="#fff" stopOpacity=".22"/><stop offset="100%" stopColor="#fff" stopOpacity=".04"/></linearGradient>
          ...
          <g clipPath="url(#sl-c)"><rect x="10" y={liqY} width="60" height={liqH+10} fill="url(#sl-l)" style={{transition:"y .5s cubic-bezier(.34,1.56,.64,1),height .5s cubic-bezier(.34,1.56,.64,1)"}}/>{pct>3&&pct<98&&<ellipse cx="40" cy={liqY} rx="28" ry="3" fill={liq.top} opacity=".35" style={{transition:"cy .5s cubic-bezier(.34,1.56,.64,1)"}}/>}</g>
      ...
      <div style={{flex:1}}>
        ...
        <input type="range" min={1} max={100} value={pct} onChange={e=>onChange(Number(e.target.value))} style={{width:"100%",height:2,appearance:"none",WebkitAppearance:"none",background:`linear-gradient(90deg,${ACC.gold} ${pct}%,${B.border} ${pct}%)`,borderRadius:2,outline:"none",cursor:"pointer"}}/>
        <div style={{display:"flex",justifyContent:"space-between",marginTop:6,fontFamily:"'Manrope',sans-serif",fontSize:9,color:T.faint}}>{["0%","25%","50%","75%","100%"].map(l=><span key={l}>{l}</span>)}</div>
        <div style={{display:"flex",gap:6,marginTop:16,flexWrap:"wrap"}}>
          {[{label:"Bontatlan",v:100},{label:"~¾",v:75},{label:"~½",v:50},{label:"~¼",v:25}].map(({label,v})=>(
            <button key={v} onClick={()=>onChange(v)} style={{background:pct===v?ACC.goldPale:"transparent",border:`1px solid ${pct===v?ACC.gold:B.border}`,color:pct===v?ACC.gold:T.muted,padding:"5px 12px",borderRadius:4,cursor:"pointer",fontFamily:"'Manrope',sans-serif",fontSize:10,transition:"all .15s"}}>{label}</button>
          ))}
        </div>
...
function BottleCompact({pct=90}) {
  const liq=bottleLiq(pct);const liqH=(pct/100)*108;const liqY=40+(108-liqH);const col=pct>=80?ACC.gold:pct>=50?"#c0924a":pct>=25?"#b07040":ACC.red;
  return(<div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}><svg width="20" height="64" viewBox="0 0 80 160" fill="none"><defs><linearGradient id="cp-l" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={liq.top} stopOpacity=".88"/><stop offset="100%" stopColor={liq.bot} stopOpacity=".8"/></linearGradient><clipPath id="cp-c"><path d="M18 40 Q14 44 12 52 L10 148 Q10 152 14 152 L66 152 Q70 152 70 148 L68 52 Q66 44 62 40 Z"/></clipPath></defs><rect x="28" y="6" width="24" height="14" rx="3" fill={B.warm} stroke={B.border} strokeWidth="1"/><path d="M30 20 L26 40 L54 40 L50 20 Z" fill={B.paper} stroke={B.border} strokeWidth="1"/><path d="M18 40 Q14 44 12 52 L10 148 Q10 152 14 152 L66 152 Q70 152 70 148 L68 52 Q66 44 62 40 Z" fill={B.canvas} stroke={B.borderDk} strokeWidth="1.5"/><g clipPath="url(#cp-c)"><rect x="10" y={liqY} width="60" height={liqH+10} fill="url(#cp-l)"/></g></svg><div><div style={{fontFamily:"'Playfair Display',serif",fontSize:15,color:col,fontWeight:600}}>{pct}%</div><div style={{fontFamily:"'Manrope',sans-serif",fontSize:9,color:T.faint,letterSpacing:1.2}}>TELE</div></div></div>);
}
  ...
  return(
    <div style={{border:`1px solid ${B.border}`,borderRadius:10,padding:"18px 22px",display:"flex",alignItems:"center",gap:22,marginBottom:28,background:B.paper}}>
      <svg width="52" height="104" viewBox="0 0 80 160" fill="none" style={{flexShrink:0}}>
        <defs><linearGradient id="dt-l" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={liq.top} stopOpacity=".9"/><stop offset="100%" stopColor={liq.bot} stopOpacity=".8"/></linearGradient><clipPath id="dt-c"><path d="M18 40 Q14 44 12 52 L10 148 Q10 152 14 152 L66 152 Q70 152 70 148 L68 52 Q66 44 62 40 Z"/></clipPath></defs>
        <rect x="28" y="6" width="24" height="14" rx="3" fill={B.warm} stroke={B.borderDk} strokeWidth="1"/><rect x="31" y="4" width="18" height="4" rx="2" fill={ACC.gold} opacity=".6"/>
        <path d="M30 20 L26 40 L54 40 L50 20 Z" fill={B.paper} stroke={B.border} strokeWidth="1"/>
        <path d="M18 40 Q14 44 12 52 L10 148 Q10 152 14 152 L66 152 Q70 152 70 148 L68 52 Q66 44 62 40 Z" fill={B.canvas} stroke={B.borderDk} strokeWidth="1.5"/>
        <g clipPath="url(#dt-c)"><rect x="10" y={liqY} width="60" height={liqH+10} fill="url(#dt-l)"/>{pct>3&&pct<98&&<ellipse cx="40" cy={liqY} rx="28" ry="3" fill={liq.top} opacity=".3"/>}</g>
      ...
      <div>
        ...
        <div style={{width:160,height:2,background:B.border,borderRadius:2}}><div style={{height:"100%",width:`${pct}%`,background:`linear-gradient(90deg,${col}60,${col})`,borderRadius:2,transition:"width .5s ease"}}/></div>
      </div>
    </div>
  );
}
      ...
      <div style={{background:B.paper,borderBottom:`1px solid ${B.border}`,padding:"24px clamp(16px,4vw,48px)"}}>
        <div style={{maxWidth:1200,margin:"0 auto"}}>
          ...
          <div style={{display:"flex",gap:5,marginTop:16,flexWrap:"wrap"}}>
            {CATS.map(c=>(<button key={c} onClick={()=>setCat(c)} style={{background:cat===c?ACC.ink:"transparent",border:`1px solid ${cat===c?ACC.ink:B.border}`,color:cat===c?B.canvas:T.muted,padding:"5px 14px",borderRadius:4,cursor:"pointer",fontFamily:"'Manrope',sans-serif",fontSize:10,letterSpacing:1,textTransform:"uppercase",fontWeight:cat===c?700:500,transition:"all .15s"}}>{c}</button>))}
          </div>
        </div>
      </div>
...
function Detail({l,u,curProfile,go,setProfileId,setActiveChatWith,onStatusChange,presenceMap={}}) {
  const [showOffer,setShowOffer]=useState(false);const [offerVal,setOfferVal]=useState("");
  const [faved,setFaved]=useState(false);const [status,setStatus]=useState(l.status||"active");
  const [showSoldModal,setShowSoldModal]=useState(false);const [msgPartners,setMsgPartners]=useState([]);
  const [selectedBuyer,setSelectedBuyer]=useState(null);const [loadingPartners,setLoadingPartners]=useState(false);
  const [buyerSearch,setBuyerSearch]=useState("");const [searchResults,setSearchResults]=useState([]);const [searchLoading,setSearchLoading]=useState(false);
  ...
  async function openSoldModal(){
    setLoadingPartners(true);setShowSoldModal(true);setBuyerSearch("");setSearchResults([]);setSelectedBuyer(null);
    ...
    data.forEach(m=>{const pid=m.from_user===curProfile.id?m.to_user:m.from_user;if(!seen.has(pid)){seen.add(pid);partners.push(pid);}});
    const{data:profiles}=await supabase.from("profiles").select("id,name,sales").in("id",partners);
    setMsgPartners(profiles||[]);setLoadingPartners(false);
  }
  async function searchBuyer(q){
    setBuyerSearch(q);if(q.trim().length<2){setSearchResults([]);return;}setSearchLoading(true);
  ...
  async function confirmSold(){
    ...
    setStatus("sold");onStatusChange?.(l.id,"sold");setShowSoldModal(false);
    try{const{data:bp}=await supabase.from("profiles").select("email,name").eq("id",selectedBuyer.id).single();if(bp?.email)await supabase.functions.invoke("clever-worker",{body:{to_email:bp.email,from_name:curProfile.name,message_text:`Sikeresen megkaptad: ${l.brand} ${l.name}\n\nKérlek értékeld az eladót!`,cta_url:"https://scentrade.vercel.app",cta_text:"Értékelés írása →"}});}catch(e){console.warn(e);}
    setReviewTarget(selectedBuyer);setShowReviewModal(true);
  }
  async function submitReview(){
    if(!reviewTarget||!curProfile)return;const text=reviewRef.current?.value?.trim()||"";
    await supabase.from("reviews").insert({from_user:curProfile.id,to_user:reviewTarget.id,rating:reviewRating,text,transaction_type:"verified",listing_id:l.id});
    ...
    <div style={{paddingTop:62,maxWidth:980,margin:"0 auto",padding:isMob?"72px 16px 48px":"80px 40px",background:B.canvas}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          ...
          {!isOwn&&status!=="sold"&&(
            <>
              ...
              {showOffer&&(<div style={{border:`1px solid ${B.border}`,borderRadius:7,padding:14,background:B.paper}}><input value={offerVal} onChange={e=>setOfferVal(e.target.value)} placeholder="Ajánlott ár (Ft)" type="number" style={{background:B.canvas,border:`1px solid ${B.border}`,color:T.body,padding:"10px 13px",borderRadius:5,width:"100%",fontFamily:"'Manrope',sans-serif",fontSize:14,marginBottom:8,boxSizing:"border-box",outline:"none"}}/><button onClick={openMsg} style={{background:ACC.goldPale,border:`1px solid ${ACC.goldMid}`,color:ACC.gold,padding:"9px",width:"100%",borderRadius:5,cursor:"pointer",fontFamily:"'Manrope',sans-serif",fontSize:11,fontWeight:700}}>Ajánlat üzenetben →</button></div>)}
            </>
          )}
          {!isOwn&&status==="sold"&&l.buyer_id===curProfile?.id&&(<button onClick={()=>{setReviewTarget(u);setShowReviewModal(true);}} style={{background:ACC.ink,border:"none",color:B.canvas,padding:"14px",borderRadius:7,cursor:"pointer",fontFamily:"'Manrope',sans-serif",fontSize:14,fontWeight:700}}>⭐ Értékelem az eladót</button>)}
          {status==="sold"&&l.buyer_id!==curProfile?.id&&(<div style={{background:ACC.greenPale,border:`1px solid ${ACC.green}30`,borderRadius:7,padding:"16px",textAlign:"center"}}><p style={{fontFamily:"'Manrope',sans-serif",fontSize:12,color:ACC.green,letterSpacing:1,fontWeight:700}}>✓ ELADVA</p></div>)}
          <div style={{border:`1px solid ${B.border}`,borderRadius:7,padding:"16px 18px",background:B.paper}}>
            ...
            {["Valódi értékelések","PM alapú egyeztetés","Hitelesített jelvény","Átverés bejelentés"].map(txt=>(<div key={txt} style={{display:"flex",gap:10,fontSize:12,color:T.muted,marginBottom:7,fontFamily:"'Manrope',sans-serif"}}><span style={{color:ACC.gold,flexShrink:0}}>✓</span>{txt}</div>))}
          </div>
        </div>
      </div>
      ...
      {showSoldModal&&(<Modal onClose={()=>setShowSoldModal(false)}>
        ...
        <div style={{marginBottom:20}}>
          <div style={{fontFamily:"'Manrope',sans-serif",fontSize:10,color:T.faint,letterSpacing:2,marginBottom:8,textTransform:"uppercase",fontWeight:700}}>Keresés névvel</div>
          <div style={{position:"relative"}}><input value={buyerSearch} onChange={e=>searchBuyer(e.target.value)} placeholder="pl. kovacs_balazs" autoComplete="off" style={{width:"100%",background:B.paper,border:`1px solid ${B.border}`,color:T.body,padding:"12px 15px",borderRadius:7,fontFamily:"'Manrope',sans-serif",fontSize:14,outline:"none",boxSizing:"border-box"}}/>{searchLoading&&<span style={{position:"absolute",right:14,top:"50%",transform:"translateY(-50%)",color:T.faint,fontSize:12}}>…</span>}</div>
          {searchResults.length>0&&<div style={{marginTop:6,display:"flex",flexDirection:"column",gap:5}}>{searchResults.map(p=>(<div key={p.id} onClick={()=>{setSelectedBuyer(p);setBuyerSearch(p.name);setSearchResults([]);}} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 13px",borderRadius:7,cursor:"pointer",background:selectedBuyer?.id===p.id?ACC.goldPale:B.paper,border:`1px solid ${selectedBuyer?.id===p.id?ACC.gold:B.border}`}}><Ava u={p} size={32}/><div><div style={{fontFamily:"'Playfair Display',serif",fontSize:15,color:T.heading}}>{p.name}</div><RankBadge sales={p.sales||0}/></div>{selectedBuyer?.id===p.id&&<span style={{color:ACC.gold,fontSize:16,marginLeft:"auto"}}>✓</span>}</div>))}</div>}
        </div>
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20}}><div style={{flex:1,height:1,background:B.border}}/><span style={{fontFamily:"'Manrope',sans-serif",fontSize:10,color:T.faint,letterSpacing:2,fontWeight:600}}>VAGY</span><div style={{flex:1,height:1,background:B.border}}/></div>
        <div style={{fontFamily:"'Manrope',sans-serif",fontSize:10,color:T.faint,letterSpacing:2,marginBottom:10,textTransform:"uppercase",fontWeight:700}}>Üzenetpartnereid</div>
        {loadingPartners?<p style={{color:T.faint,fontFamily:"'Manrope',sans-serif",fontSize:12,padding:"16px 0"}}>Betöltés...</p>:(
          <div style={{display:"flex",flexDirection:"column",gap:6,marginBottom:22,maxHeight:220,overflowY:"auto"}}>
            {msgPartners.length===0&&<p style={{color:T.faint,fontFamily:"'Manrope',sans-serif",fontSize:12,padding:"10px 0"}}>Még nincs üzenetpartnered.</p>}
            {msgPartners.map(p=>(<div key={p.id} onClick={()=>{setSelectedBuyer(p);setBuyerSearch("");setSearchResults([]);}} style={{display:"flex",alignItems:"center",gap:12,padding:"11px 13px",borderRadius:7,cursor:"pointer",background:selectedBuyer?.id===p.id?ACC.goldPale:B.paper,border:`1px solid ${selectedBuyer?.id===p.id?ACC.gold:B.border}`,transition:"all .15s"}}><Ava u={p} size={34}/><div style={{flex:1}}><div style={{fontFamily:"'Playfair Display',serif",fontSize:15,color:T.heading}}>{p.name}</div><RankBadge sales={p.sales||0}/></div>{selectedBuyer?.id===p.id&&<span style={{color:ACC.gold,fontSize:16}}>✓</span>}</div>))}
          </div>
        )}
        {selectedBuyer&&<div style={{background:ACC.greenPale,border:`1px solid ${ACC.green}30`,borderRadius:7,padding:"11px 14px",marginBottom:14,display:"flex",alignItems:"center",gap:12}}><Ava u={selectedBuyer} size={30}/><div><div style={{fontFamily:"'Manrope',sans-serif",fontSize:9,color:ACC.green,letterSpacing:1.5,marginBottom:2,fontWeight:700}}>KIVÁLASZTOTT VEVŐ</div><div style={{fontFamily:"'Playfair Display',serif",fontSize:15,color:T.heading}}>{selectedBuyer.name}</div></div><button onClick={()=>setSelectedBuyer(null)} style={{marginLeft:"auto",background:"none",border:"none",color:T.faint,cursor:"pointer",fontSize:18}}>×</button></div>}
        <button onClick={confirmSold} disabled={!selectedBuyer} style={{background:selectedBuyer?ACC.green:B.warm,border:"none",color:selectedBuyer?"#fff":T.faint,padding:"14px",width:"100%",borderRadius:7,cursor:selectedBuyer?"pointer":"not-allowed",fontFamily:"'Manrope',sans-serif",fontSize:14,fontWeight:700}}>
          {selectedBuyer?`✓ Eladva – ${selectedBuyer.name}`:"Válassz vevőt a megerősítéshez"}
        </button>
      </Modal>)}
      {showReviewModal&&reviewTarget&&(<Modal onClose={()=>setShowReviewModal(false)}>
        <h2 style={{fontFamily:"'Playfair Display',serif",fontSize:26,color:T.heading,fontWeight:400,marginBottom:6}}>Értékeld: {reviewTarget.name}</h2>
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:18}}><Pill text="✓ Verified Purchase" bg={ACC.greenPale} col={ACC.green}/></div>
        <div style={{marginBottom:18}}><Stars v={reviewRating} size={30} interactive onChange={setReviewRating}/></div>
        <textarea ref={reviewRef} defaultValue="" rows={4} placeholder="Hogyan ment az üzlet?" style={{background:B.paper,border:`1px solid ${B.border}`,color:T.body,padding:"13px 15px",borderRadius:7,width:"100%",fontFamily:"'Manrope',sans-serif",fontSize:14,resize:"vertical",outline:"none",boxSizing:"border-box",marginBottom:16,lineHeight:1.7}}/>
        <button onClick={submitReview} style={{background:ACC.ink,border:"none",color:B.canvas,padding:"14px",width:"100%",borderRadius:7,cursor:"pointer",fontFamily:"'Manrope',sans-serif",fontSize:14,fontWeight:700}}>Értékelés elküldése →</button>
      </Modal>)}
    </div>
  );
}

function Profile({pu,curProfile,go,listings,setActiveChatWith,onSignOut,presenceMap={}}) {
  const [tab,setTab]=useState("listings");const [reviews,setReviews]=useState([]);
  ...
  async function submitReview(){if(!curProfile)return;await supabase.from("reviews").insert({from_user:curProfile.id,to_user:pu.id,rating:myRating,text:myText,transaction_type:"full"});setShowRM(false);setMyText("");const{data}=await supabase.from("reviews").select("*").eq("to_user",pu.id);setReviews(data||[]);}
  async function addWish(){const val=wishRef.current?.value?.trim();if(!val||!curProfile)return;const[brand,...rest]=val.split(" ");const{data}=await supabase.from("wishlists").insert({user_id:curProfile.id,brand,name:rest.join(" ")||brand,raw:val}).select().single();if(data){setWishlist(p=>[data,...p]);wishRef.current.value="";}}
  async function removeWish(id){await supabase.from("wishlists").delete().eq("id",id);setWishlist(p=>p.filter(w=>w.id!==id));}
  if(!pu)return null;
    ...
    <div style={{paddingTop:62,maxWidth:980,margin:"0 auto",padding:isMob?"72px 16px 40px":"80px 40px",background:B.canvas}}>
      <div style={{display:"flex",gap:isMob?16:32,alignItems:"flex-start",marginBottom:48,flexWrap:"wrap",paddingBottom:40,borderBottom:`1px solid ${B.border}`}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          {!isOwn&&curProfile&&(<><button onClick={()=>{setActiveChatWith(pu.id);go("messages");}} style={{background:ACC.ink,border:"none",color:B.canvas,padding:"10px 22px",borderRadius:6,cursor:"pointer",fontFamily:"'Manrope',sans-serif",fontSize:12,fontWeight:700}}>✉ ÜZENET</button><button onClick={()=>setShowRM(true)} style={{background:"transparent",border:`1px solid ${B.borderDk}`,color:T.body,padding:"10px 22px",borderRadius:6,cursor:"pointer",fontFamily:"'Manrope',sans-serif",fontSize:12,fontWeight:600}}>⭐ ÉRTÉKELÉS</button></>)}
      ...
      {tab==="wishlist"&&(
        <div>
          <div style={{border:`1px solid ${B.border}`,borderRadius:8,padding:"22px 26px",marginBottom:24,background:B.paper}}><p style={{fontFamily:"'Playfair Display',serif",fontSize:20,color:T.heading,marginBottom:8}}>🌟 Kívánlista</p><p style={{fontFamily:"'Manrope',sans-serif",fontSize:12,color:T.muted,lineHeight:1.8}}>Ha valaki feltölti valamelyik keresett parfümodet, automatikus értesítést kapsz e-mailben.</p></div>
          {isOwn&&(<div style={{display:"flex",gap:10,marginBottom:22}}><input ref={wishRef} placeholder="pl. Creed Aventus  vagy  Dior Sauvage" defaultValue="" autoComplete="off" autoCorrect="off" spellCheck="false" style={{flex:1,background:B.paper,border:`1px solid ${B.border}`,color:T.body,padding:"12px 15px",borderRadius:7,fontFamily:"'Manrope',sans-serif",fontSize:14,outline:"none"}}/><button onClick={addWish} style={{background:ACC.ink,border:"none",color:B.canvas,padding:"12px 20px",borderRadius:7,cursor:"pointer",fontFamily:"'Manrope',sans-serif",fontSize:12,fontWeight:700,whiteSpace:"nowrap"}}>+ Hozzáad</button></div>)}
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {wishlist.map(w=>(<div key={w.id} style={{border:`1px solid ${B.border}`,borderRadius:8,padding:"14px 18px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:14,background:B.paper}}><div style={{display:"flex",alignItems:"center",gap:14}}><span style={{fontSize:20}}>🌟</span><div><div style={{fontFamily:"'Playfair Display',serif",fontSize:16,color:T.heading}}>{w.raw}</div><div style={{fontFamily:"'Manrope',sans-serif",fontSize:9,color:T.faint,marginTop:2,letterSpacing:1.2,fontWeight:600}}>ÉRTESÍTÉST KÉREK HA MEGJELENIK</div></div></div>{isOwn&&<button onClick={()=>removeWish(w.id)} style={{background:"transparent",border:`1px solid ${ACC.red}25`,color:ACC.red,padding:"5px 11px",borderRadius:4,cursor:"pointer",fontFamily:"'Manrope',sans-serif",fontSize:10,fontWeight:600}}>Töröl</button>}</div>))}
...
function Messages({curProfile,activeChatWith,setActiveChatWith,presenceMap={}}) {
  const [convs,setConvs]=useState([]);const [partnerProfiles,setPartnerProfiles]=useState({});
  const [chat,setChat]=useState([]);const [newMsg,setNewMsg]=useState("");const [mobileView,setMobileView]=useState("list");
  const bottomRef=useRef(null);const inputRef=useRef(null);
  useEffect(()=>{
    if(!curProfile?.id)return;loadConvs();
    const sub=supabase.channel("msg-rt-"+curProfile.id)
      .on("postgres_changes",{event:"INSERT",schema:"public",table:"messages",filter:`to_user=eq.${curProfile.id}`},payload=>{
        if(activeChatWith===payload.new.from_user){supabase.from("messages").update({read:true,read_at:new Date().toISOString()}).eq("id",payload.new.id).then(()=>loadChat(activeChatWith));}else loadConvs();
      })
  ...
  useEffect(()=>{if(activeChatWith){loadChat(activeChatWith);setMobileView("chat");}},[activeChatWith]);
  useEffect(()=>{bottomRef.current?.scrollIntoView({behavior:"smooth"});},[chat]);
  async function loadConvs(){
    const{data}=await supabase.from("messages").select("*").or(`from_user.eq.${curProfile.id},to_user.eq.${curProfile.id}`).order("created_at",{ascending:false});
    if(!data)return;const seen=new Set(),cs=[];
    ...
    setConvs(cs);const ids=[...seen];
    if(ids.length){const{data:pd}=await supabase.from("profiles").select("*").in("id",ids);if(pd){const m={};pd.forEach(p=>{m[p.id]=p;});setPartnerProfiles(m);}}
  }
  ...
  async function send(){
    ...
    const{error}=await supabase.from("messages").insert({from_user:curProfile.id,to_user:activeChatWith,text:txt,read:false,delivered:true});
    if(error){console.error(error);return;}
    try{const partner=partnerProfiles[activeChatWith];if(partner?.email)await supabase.functions.invoke("clever-worker",{body:{to_email:partner.email,to_name:partner.name,from_name:curProfile.name,message_text:txt}});}catch(e){console.warn(e);}
  ...
  const listPane=(
    <div style={{background:B.paper,borderRight:`1px solid ${B.border}`,width:isMobile?"100%":300,flexShrink:0,display:(!isMobile||mobileView==="list")?"flex":"none",flexDirection:"column"}}>
      <div style={{padding:"20px",borderBottom:`1px solid ${B.border}`,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        ...
        {totalUnread>0&&<span style={{background:ACC.gold,borderRadius:10,padding:"2px 8px",fontFamily:"'Manrope',sans-serif",fontSize:10,color:"#fff",fontWeight:700}}>{totalUnread}</span>}
      </div>
      <div style={{overflowY:"auto",flex:1}}>
        ...
        {convs.map(({partnerId,lastMsg})=>{
          const u=partnerProfiles[partnerId];const name=u?.name||"…";
          const active=activeChatWith===partnerId;const isUnread=lastMsg.to_user===curProfile.id&&!lastMsg.read;
          return(
            <div key={partnerId} onClick={()=>{setActiveChatWith(partnerId);setMobileView("chat");}}
              style={{padding:"14px 18px",cursor:"pointer",display:"flex",gap:12,alignItems:"center",background:active?B.warm:"transparent",borderLeft:active?`3px solid ${ACC.gold}`:isUnread?`3px solid ${ACC.gold}55`:"3px solid transparent",borderBottom:`1px solid ${B.border}`}}>
              <div style={{position:"relative",flexShrink:0}}>
                <Ava u={u||{name}} size={40}/>
                <span style={{position:"absolute",bottom:0,right:0}}><OnlineBadge presenceMap={presenceMap} userId={partnerId}/></span>
              </div>
              <div style={{flex:1,minWidth:0}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:3}}>
                  <span style={{fontFamily:"'Manrope',sans-serif",fontSize:13,color:T.heading,fontWeight:isUnread?700:500}}>{name}</span>
                  <span style={{fontFamily:"'Manrope',sans-serif",fontSize:10,color:T.faint}}>{relTime(lastMsg.created_at)}</span>
                </div>
                <div style={{fontSize:12,color:isUnread?T.muted:T.faint,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",fontFamily:"'Manrope',sans-serif",fontWeight:isUnread?600:400}}>
  ...
  const chatPane=(
    <div style={{flex:1,display:(!isMobile||mobileView==="chat")?"flex":"none",flexDirection:"column",background:B.canvas,minWidth:0}}>
      {activeChatWith?(
        <>
          <div style={{padding:"14px 20px",borderBottom:`1px solid ${B.border}`,display:"flex",alignItems:"center",gap:12,background:B.paper,flexShrink:0}}>
            {isMobile&&<button onClick={()=>setMobileView("list")} style={{background:"none",border:"none",color:T.muted,fontSize:22,cursor:"pointer",padding:"0 8px 0 0"}}>‹</button>}
            <Ava u={activePartner||{name:"?"}} size={38}/>
            <div style={{flex:1}}>
              <div style={{fontFamily:"'Playfair Display',serif",fontSize:18,color:T.heading}}>{activePartner?.name||"…"}</div>
              <div style={{marginTop:3}}><OnlineBadge presenceMap={presenceMap} userId={activeChatWith} showLabel={true}/></div>
            </div>
            ...
            {chat.map((m,i)=>{
              ...
              return(
                <div key={m.id||i} style={{display:"flex",justifyContent:me?"flex-end":"flex-start"}}>
                  <div style={{maxWidth:"76%",padding:"10px 14px",background:me?B.warm:B.paper,border:`1px solid ${me?B.borderDk:B.border}`,borderRadius:me?"12px 12px 3px 12px":"12px 12px 12px 3px"}}>
                    ...
                    <div style={{display:"flex",alignItems:"center",justifyContent:"flex-end",gap:5,marginTop:4}}>
                      ...
                      {me&&m.read&&<span style={{fontFamily:"'Manrope',sans-serif",fontSize:9,color:ACC.gold}}>✓✓ látta</span>}
                    </div>
                  </div>
                </div>
              );
          ...
          <div style={{padding:"12px 16px",borderTop:`1px solid ${B.border}`,display:"flex",gap:10,background:B.paper,flexShrink:0}}>
            <input ref={inputRef} value={newMsg} onChange={e=>setNewMsg(e.target.value)} onKeyDown={e=>e.key==="Enter"&&!e.shiftKey&&send()} placeholder="Írj üzenetet..."
            ...
            <button onClick={send} style={{background:ACC.ink,border:"none",color:B.canvas,padding:"12px 18px",borderRadius:8,cursor:"pointer",fontFamily:"'Manrope',sans-serif",fontWeight:700,fontSize:16,flexShrink:0}}>→</button>
          </div>
        </>
      ):(
...
function Sell({curProfile,go,setListings,showToast}) {
  ...
  async function submit(){
    ...
    try{const{data:matches}=await supabase.from("wishlists").select("*, profiles(email, name)").neq("user_id",curProfile.id).or(`raw.ilike.%${brand}%,raw.ilike.%${name}%`);if(matches?.length>0){for(const match of matches){if(!match.profiles?.email)continue;await supabase.functions.invoke("clever-worker",{body:{to_email:match.profiles.email,from_name:"Scentrade",message_text:`🌟 Megjelent: ${brand} ${name}`,subject_override:`🌟 Megjelent: ${brand} ${name}`,cta_url:"https://scentrade.vercel.app",cta_text:"Megnézem →"}});}}}catch(e){console.warn(e);}
    showToast("Hirdetés sikeresen közzétéve!","success");setTimeout(()=>go("market"),900);
  }
    ...
    <div style={{paddingTop:62,maxWidth:700,margin:"0 auto",padding:"72px 16px 80px",background:B.canvas}}>
      ...
      <div style={{marginBottom:24}}>
        <div onClick={()=>setSSwap(v=>!v)} style={{display:"flex",gap:14,alignItems:"center",cursor:"pointer"}}>
          ...
          <div><div style={{fontFamily:"'Manrope',sans-serif",fontSize:13,color:sSwap?"#7a5ab0":T.body,fontWeight:600}}>Csere is érdekel</div><div style={{fontFamily:"'Manrope',sans-serif",fontSize:11,color:T.faint,marginTop:2}}>Más parfümre is cserélném</div></div>
        </div>
      </div>
...
function Login({go,showToast}) {
  ...
  async function doLogin(){
    if(!ve(email)){showToast("Adj meg érvényes email címet!","error");return;}if(pass.length<6){showToast("A jelszónak legalább 6 karakter kell!","error");return;}
    setLoading(true);const{error}=await supabase.auth.signInWithPassword({email,password:pass});setLoading(false);
    if(error){if(error.message.includes("Invalid login")||error.message.includes("invalid_credentials"))showToast("Hibás email cím vagy jelszó.","error");else if(error.message.includes("Email not confirmed"))showToast("Erősítsd meg az email címed!","info");else showToast(error.message,"error");}
    else go("home");
  }
  async function doRegister(){
    if(!name.trim()){showToast("Add meg a felhasználóneved!","error");return;}if(!ve(email)){showToast("Adj meg érvényes email címet!","error");return;}
    if(pass.length<6){showToast("A jelszónak legalább 6 karakter kell!","error");return;}if(pass!==passConfirm){showToast("A két jelszó nem egyezik!","error");return;}if(!tos){showToast("El kell fogadnod az ÁSZF-et!","error");return;}
    setLoading(true);const{data:sd,error}=await supabase.auth.signUp({email,password:pass,options:{data:{name,location:loc}}});
    ...
    setLoading(false);if(sd.session){showToast("Sikeres regisztráció! Üdv a Scentrade-n!","success");go("home");}else setRegState("confirm");
  }
  const inp={background:B.canvas,border:`1px solid ${B.border}`,color:T.body,padding:"13px 15px",borderRadius:7,fontFamily:"'Manrope',sans-serif",fontSize:16,width:"100%",boxSizing:"border-box",outline:"none",marginBottom:12};
  ...
  if(regState==="confirm")return(
    <div style={{paddingTop:62,minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:B.canvas}}>
      <div style={{border:`1px solid ${B.border}`,borderRadius:12,padding:"clamp(28px,6vw,56px) clamp(20px,5vw,48px)",width:"100%",maxWidth:440,textAlign:"center",background:B.paper,boxShadow:"0 8px 40px rgba(0,0,0,.06)"}}>
        ...
        <button onClick={()=>{setMode("login");setRegState("idle");}} style={{background:ACC.ink,border:"none",color:B.canvas,padding:"14px",width:"100%",borderRadius:7,cursor:"pointer",fontFamily:"'Manrope',sans-serif",fontSize:14,fontWeight:700,marginBottom:12}}>Bejelentkezéshez →</button>
        <p style={{color:T.faint,fontSize:11,fontFamily:"'Manrope',sans-serif",cursor:"pointer",fontWeight:600}} onClick={()=>setRegState("idle")}>Vissza a regisztrációhoz</p>
      </div>
    </div>
  );
  return(
    <div style={{paddingTop:62,minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",padding:"72px 16px 40px",background:B.canvas}}>
      <div style={{border:`1px solid ${B.border}`,borderRadius:12,padding:"clamp(28px,6vw,56px) clamp(20px,5vw,48px)",width:"100%",maxWidth:440,boxShadow:"0 8px 40px rgba(0,0,0,.06)",background:B.paper}}>
        ...
        <div style={{display:"flex",background:B.warm,borderRadius:7,padding:3,marginBottom:32}}>
          {[["login","Belépés"],["register","Regisztráció"]].map(([m,l])=>(<button key={m} onClick={()=>sw(m)} style={{flex:1,padding:"9px",borderRadius:5,border:"none",cursor:"pointer",background:mode===m?B.canvas:"transparent",color:mode===m?T.heading:T.faint,fontFamily:"'Manrope',sans-serif",fontSize:12,fontWeight:mode===m?700:500,transition:"all .15s",boxShadow:mode===m?"0 1px 4px rgba(0,0,0,.06)":"none"}}>{l}</button>))}
        </div>
        ...
        <label style={lbl}>Email cím *</label><input value={email} onChange={e=>setEmail(e.target.value)} placeholder="pelda@email.hu" type="email" style={inp}/>
        <label style={lbl}>Jelszó *</label><input value={pass} onChange={e=>setPass(e.target.value)} placeholder="••••••••" type="password" onKeyDown={e=>e.key==="Enter"&&mode==="login"&&doLogin()} style={{...inp,marginBottom:mode==="register"?12:20}}/>
        {mode==="register"&&(
          <>
            <label style={lbl}>Jelszó mégegyszer *</label>
            <input value={passConfirm} onChange={e=>setPCfm(e.target.value)} placeholder="••••••••" type="password" onKeyDown={e=>e.key==="Enter"&&doRegister()} style={{...inp,border:`1px solid ${passConfirm&&passConfirm!==pass?ACC.red+"55":B.border}`}}/>
            {passConfirm&&passConfirm!==pass&&<p style={{color:ACC.red,fontFamily:"'Manrope',sans-serif",fontSize:11,marginTop:-8,marginBottom:12}}>A jelszavak nem egyeznek</p>}
            <div style={{marginBottom:24}}>
              <label style={{display:"flex",gap:12,alignItems:"flex-start",cursor:"pointer"}}>
                <div onClick={()=>setTos(v=>!v)} style={{width:20,height:20,borderRadius:4,flexShrink:0,marginTop:1,background:tos?ACC.goldPale:"transparent",border:`1.5px solid ${tos?ACC.gold:B.borderDk}`,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",transition:"all .15s"}}>{tos&&<span style={{color:ACC.gold,fontSize:12,fontWeight:700}}>✓</span>}</div>
        ...
        <p style={{color:T.faint,fontSize:11,fontFamily:"'Manrope',sans-serif",textAlign:"center",cursor:"pointer",letterSpacing:.5,fontWeight:600}} onClick={()=>sw(mode==="login"?"register":"login")}>{mode==="login"?"Még nincs fiókod? Regisztrálj":"Már van fiókod? Lépj be"}</p>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  const [profileId,setProfileId]=useState(()=>sessionStorage.getItem("sc_profileId")||null);
  const [activeChatWith,setACW]=useState(null);const [unread,setUnread]=useState(0);
  const{show:showToast,ToastContainer}=useToast();

  useEffect(()=>{sessionStorage.setItem("sc_page",page);},[page]);
  useEffect(()=>{if(selId)sessionStorage.setItem("sc_selId",selId);else sessionStorage.removeItem("sc_selId");},[selId]);
  useEffect(()=>{if(profileId)sessionStorage.setItem("sc_profileId",profileId);else sessionStorage.removeItem("sc_profileId");},[profileId]);
  ...
  if(loading)return(<div style={{background:B.canvas,minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center"}}><span style={{fontFamily:"'Playfair Display',serif",fontSize:28,color:ACC.gold,letterSpacing:4,animation:"pulse 1.2s ease infinite"}}>SCENTRADE</span></div>);
    ...
    <div style={{background:B.canvas,minHeight:"100vh",color:T.body}}>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        ...
        button{transition:opacity .15s,background .15s,border-color .15s,color .15s;}button:hover{opacity:.85;}
      ...
      {page==="detail"      &&selListing&&<Detail l={selListing} u={allProfiles[selListing.user_id]} curProfile={profile} go={go} setProfileId={setProfileId} setActiveChatWith={setACW} onStatusChange={handleStatusChange} presenceMap={presenceMap}/>}
      {page==="profile"     &&profileUser&&<Profile pu={profileUser} curProfile={profile} go={go} listings={listings} setActiveChatWith={setACW} onSignOut={signOut} presenceMap={presenceMap}/>}
      {page==="profile_own" &&profile    &&<Profile pu={profile} curProfile={profile} go={go} listings={listings} setActiveChatWith={setACW} onSignOut={signOut} presenceMap={presenceMap}/>}
      {page==="messages"    &&profile    &&<Messages curProfile={profile} activeChatWith={activeChatWith} setActiveChatWith={setACW} presenceMap={presenceMap}/>}
